  ...
  return (
    <section className="rounded-[1.6rem] border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="text-sm uppercase tracking-[.18em] text-zinc-400">{ar ? "مصادر الرصد" : "MONITORING SOURCES"}</div>
          <h2 className="mt-1 text-xl ">{ar ? "الحسابات والمنصات" : "Accounts & platforms"}</h2>
        </div>
        <button onClick={save} disabled={loading || saving} className="rounded-full bg-[#330033] px-5 py-2.5 text-base text-white disabled:opacity-50">
          ...
          <label key={p.id} className="rounded-2xl border border-zinc-200 bg-zinc-50/50 p-3 transition focus-within:border-[#330033] focus-within:bg-white">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {status(p.id)}
                <span className="text-[11px] uppercase tracking-wide text-zinc-400">{p.provider === "Bright Data" ? "BD" : "ED"}</span>
              </div>
            </div>
      ...
      {message && <div className="mt-4 rounded-xl bg-emerald-50 px-4 py-2 text-sm text-emerald-700">{message}</div>}
