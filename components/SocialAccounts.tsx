...
const PLATFORMS: {
  ...
  },
  {
    id: "facebook",
    label: "Facebook",
    placeholder: "Page username or public URL",
  ...
  },
  {
    id: "google_maps",
    label: "Google Maps",
    placeholder: "Full Google Maps place URL",
    provider: "Bright Data",
  },
  {
    id: "reddit",
    label: "Reddit",
    placeholder: "r/community or reddit user URL",
    provider: "EnsembleData",
  },
  {
    id: "snapchat",
...
export default function SocialAccounts({
  ...
  const ar = locale === "ar";
  const emptyValues = () =>
    Object.fromEntries(PLATFORMS.map((p) => [p.id, ""])) as Record<
      Platform,
      string
    >;
  ...
  function status(platform: Platform) {
    ...
    if (!raw)
      return (
        <span className="h-2 w-2 rounded-full bg-zinc-300" title="Not synced" />
      );
    ...
    return (
      <span
        ...
        title={String(statuses[platform]?.last_sync_error || "Sync failed")}
      />
    );
  }
  ...
  return (
    <section className="rounded-[1.6rem] border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="text-sm uppercase tracking-[.18em] text-zinc-400">
            {ar ? "مصادر الرصد" : "MONITORING SOURCES"}
          </div>
          <h2 className="mt-1 text-xl ">
            {ar ? "الحسابات والمنصات" : "Accounts & platforms"}
          </h2>
        </div>
        <button
          onClick={save}
          disabled={loading || saving}
          className="rounded-full bg-[#330033] px-5 py-2.5 text-base text-white disabled:opacity-50"
        >
          {saving
            ? ar
              ? "جارٍ الحفظ..."
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {PLATFORMS.map((p) => (
          <label
            key={p.id}
            className="rounded-2xl border border-zinc-200 bg-zinc-50/50 p-3 transition focus-within:border-[#330033] focus-within:bg-white"
          >
