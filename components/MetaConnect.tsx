...
const CONFIG_ID =
  process.env.NEXT_PUBLIC_META_CONFIG_ID || "1866651731165144";
      ...
      async (response) => {
        ...
        if (!token) {
          ...
          setResult({
            ok: false,
            error: ar
              ? "لم يكتمل تسجيل الدخول أو لم يتم منح الصلاحيات."
              : "Login did not complete or permissions were not granted.",
          });
          return;
        }
        ...
        try {
          ...
          setResult({
            ok: false,
            error: ar
              ? "تعذر اختبار الاتصال مع Meta."
              : "Could not test the Meta connection.",
          });
        } finally {
          setBusy(false);
        }
