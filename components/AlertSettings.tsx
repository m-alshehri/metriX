    ...
    <section id="alert-settings" className="mt-10">
      <div className="rounded-[2rem] border bg-white p-6 shadow-sm">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-base leading-6 text-zinc-500">
            {ar
        ...
        {status === "saved" && (
          <div className="mt-5 rounded-2xl bg-emerald-50 px-4 py-3 text-base font-semibold text-emerald-700">
            {ar ? "تم حفظ الإعدادات بنجاح." : "Settings saved successfully."}
          </div>
        )}

        {status === "failed" && (
          <div className="mt-5 rounded-2xl bg-red-50 px-4 py-3 text-base font-semibold text-red-700">
        ...
        <form action={saveAlertSettings} className="mt-6 space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="flex cursor-pointer items-start gap-3 rounded-2xl border p-4">
              ...
              <span><span className="block font-extrabold">{ar ? "الملخص التنفيذي اليومي" : "Daily executive summary"}</span><span className="mt-1 block text-base leading-6 text-zinc-500">{ar ? "إنشاء ملخص AI يومي بعد اكتمال جمع البيانات." : "Generate a daily AI brief after collection completes."}</span></span>
            </label>
            <label className="flex cursor-pointer items-start gap-3 rounded-2xl border p-4">
              <input type="checkbox" name="anomaly_alerts_enabled" defaultChecked={anomalyAlertsEnabled} className="mt-1 h-4 w-4" />
              <span><span className="block font-extrabold">{ar ? "كشف الشذوذ والارتفاعات" : "Anomaly & spike detection"}</span><span className="mt-1 block text-base leading-6 text-zinc-500">{ar ? "تنبيه عند الارتفاع غير المعتاد أو تغير السلبية." : "Alert on unusual volume or sentiment shifts."}</span></span>
            </label>
          </div>

          <div className="grid gap-5 md:grid-cols-2">
            <label className="block"><span className="text-base font-extrabold">{ar ? "مدة الاحتفاظ بالبيانات (يوم)" : "Data retention (days)"}</span><input type="number" name="retention_days" min="30" max="3650" defaultValue={retentionDays} className="mt-2 w-full rounded-2xl border bg-white px-4 py-3" /><span className="mt-2 block text-sm text-zinc-500">{ar ? "الافتراضي 365 يوماً. القيم القديمة تُحذف آلياً." : "Default 365 days. Older records are automatically removed."}</span></label>
            <label className="block"><span className="text-base font-extrabold">{ar ? "نافذة المقارنة (يوم)" : "Comparison window (days)"}</span><input type="number" name="comparison_window_days" min="1" max="90" defaultValue={comparisonWindowDays} className="mt-2 w-full rounded-2xl border bg-white px-4 py-3" /><span className="mt-2 block text-sm text-zinc-500">{ar ? "تستخدم لمقارنة الأداء بالفترة السابقة." : "Used for period-over-period performance comparison."}</span></label>
          </div>

          <div className="flex justify-end">
            <button
              type="submit"
              className="rounded-full bg-metrix-900 px-6 py-3 font-extrabold text-white transition hover:opacity-90"
            >
