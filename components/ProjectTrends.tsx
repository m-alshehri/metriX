...
export default function ProjectTrends({ mentions, locale }: { mentions: Mention[]; locale: string }) {
  ...
  return (
    <section className="mt-10">
      <div className="text-sm uppercase tracking-[0.2em] text-metrix-700">{ar ? "ذكاء الاتجاهات" : "TREND INTELLIGENCE"}</div>
      <h2 className="mt-2 text-4xl ">{ar ? "اتجاهات نشاط الحسابات" : "Account Activity Trends"}</h2>

      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-[2rem] border bg-white p-6 shadow-sm">
          <h3 className="">{ar ? "حجم النشاط بمرور الوقت" : "Activity volume over time"}</h3>
        ...
        <div className="rounded-[2rem] border bg-white p-6 shadow-sm">
          <h3 className="">{ar ? "المواضيع المتكررة" : "Trending terms"}</h3>
          <div className="mt-4 flex flex-wrap gap-2">
            {topTerms.map(([term,count])=>(
              <span key={term} className="rounded-full bg-metrix-50 px-3 py-2 text-base text-metrix-900">{term} · {count}</span>
            ))}
          </div>
