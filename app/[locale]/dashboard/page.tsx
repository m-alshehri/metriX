      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex gap-2">
            <Link href={`/${locale}/projects/new`} className="rounded-full bg-[#330033] px-5 py-2.5 text-base font-extrabold text-white">
              + {t.dashboard.newProject}
            </Link>
            <form action={signOut}>
              <input type="hidden" name="locale" value={locale} />
              <button className="rounded-full border border-zinc-300 bg-white px-5 py-2.5 text-base font-semibold text-zinc-700">
                {t.dashboard.logout}
              </button>
            </form>
          </div>
        ...
        <div className="mt-10 flex items-center justify-between">
          <h2 className="text-2xl font-extrabold">{t.dashboard.projectsTitle}</h2>
          <span className="rounded-full bg-white px-3 py-1 text-sm font-semibold text-zinc-500 shadow-sm ring-1 ring-zinc-200">
        ...
        {(projects || []).length === 0 ? (
          <div className="mt-6 rounded-[2rem] border border-dashed border-zinc-300 bg-white p-12 text-center">
            ...
            <h3 className="mt-3 font-extrabold">{ar ? "أنشئ أول مشروع" : "Create your first project"}</h3>
