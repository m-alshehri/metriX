    ...
    <main className="overflow-hidden bg-white">
      <section className="relative border-b border-zinc-100">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-6 pb-20 pt-20 lg:grid-cols-[1.05fr_.95fr] lg:pb-28 lg:pt-24">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#660066]/15 bg-[#660066]/5 px-4 py-2 text-sm tracking-[.08em] text-[#660066]">
              <Sparkles size={15} />
              {copy.eyebrow}
            </div>

            <h1 className="mt-7 max-w-4xl text-6xl leading-[1.02] tracking-[-0.055em] text-[#330033] sm:text-7xl lg:text-8xl">
            ...
            <p className="mt-7 max-w-2xl text-xl leading-8 text-zinc-600">{copy.intro}</p>

            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#request-demo" className="inline-flex items-center gap-2 rounded-full bg-[#330033] px-6 py-3.5 text-base text-white transition hover:bg-[#660066]">
                {copy.demo} <ArrowRight size={17} className={ar ? "rotate-180" : ""} />
              </a>
              <a href={`/${locale}/login`} className="rounded-full border border-zinc-300 bg-white px-6 py-3.5 text-base text-zinc-800 transition hover:border-[#660066] hover:text-[#660066]">
                {copy.login}
              </a>
            </div>
          </div>
            ...
            <div className="rounded-[2rem] border border-zinc-200 bg-white p-4 shadow-[0_30px_90px_rgba(51,0,51,0.14)]">
              <div className="rounded-[1.5rem] bg-zinc-50 p-5">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-[12px] uppercase tracking-[.2em] text-[#660066]">metriX intelligence</div>
                    <div className="mt-1 text-2xl text-[#330033]">{ar ? "نظرة عامة على العلامة" : "Brand overview"}</div>
                  </div>
                  <div className="rounded-full bg-white px-3 py-2 text-sm text-zinc-500 shadow-sm">{ar ? "آخر 30 يوم" : "Last 30 days"}</div>
                </div>
                ...
                <div className="mt-5 grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={label} className="rounded-2xl border border-zinc-200 bg-white p-4">
                      ...
                      <div className="mt-1 text-[12px] text-emerald-600">{delta}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-[1.2fr_.8fr]">
                  ...
                  <div className="rounded-2xl border border-zinc-200 bg-white p-4">
                    <div className="text-sm ">{ar ? "المشاعر" : "Sentiment"}</div>
                    <div className="mt-5 space-y-4">
                      <div>
                        <div className="flex justify-between text-[13px] "><span>{ar ? "إيجابي" : "Positive"}</span><span>68%</span></div>
                        <div className="mt-1.5 h-2 rounded-full bg-zinc-100"><div className="h-full w-[68%] rounded-full bg-emerald-500" /></div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[13px] "><span>{ar ? "محايد" : "Neutral"}</span><span>23%</span></div>
                        <div className="mt-1.5 h-2 rounded-full bg-zinc-100"><div className="h-full w-[23%] rounded-full bg-zinc-400" /></div>
                      </div>
                      <div>
                        <div className="flex justify-between text-[13px] "><span>{ar ? "سلبي" : "Negative"}</span><span>9%</span></div>
        ...
        <div className="mx-auto max-w-7xl px-6 pb-12">
          <p className="text-center text-sm uppercase tracking-[.16em] text-zinc-400">{copy.trusted}</p>
      ...
      <section id="solutions" className="bg-zinc-950 py-24 text-white">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-6 lg:grid-cols-2">
          <div>
            <div className="text-sm uppercase tracking-[.18em] text-fuchsia-300">{copy.alerts}</div>
            <h2 className="mt-4 text-5xl tracking-[-0.045em] sm:text-6xl">{copy.neverMiss}</h2>
            <p className="mt-5 max-w-xl text-xl leading-8 text-zinc-300">{copy.alertsP}</p>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl">
            <div className="rounded-[1.4rem] bg-white p-5 text-[#330033]">
              <div className="flex items-center justify-between">
                <div className="">{ar ? "تنبيه ذكي" : "Smart alert"}</div>
                <div className="rounded-full bg-red-50 px-3 py-1 text-[13px] text-red-600">{ar ? "تغيّر مهم" : "Important shift"}</div>
              </div>
              <div className="mt-5 rounded-2xl bg-zinc-50 p-5">
                <div className="text-sm text-zinc-400">{ar ? "ارتفاع في المحادثات" : "Conversation spike detected"}</div>
                <div className="mt-2 text-4xl ">+164%</div>
      ...
      <section id="ai" className="mx-auto grid max-w-7xl items-center gap-14 px-6 py-24 lg:grid-cols-2">
        <div className="order-2 lg:order-1">
          <div className="rounded-[2rem] border border-zinc-200 bg-zinc-50 p-5">
            <div className="rounded-[1.4rem] bg-white p-5 shadow-sm">
              <div className="flex items-center gap-2 text-[#660066]"><BrainCircuit size={20} /> AI Insights</div>
              <div className="mt-5 space-y-3">
                {[
                  ...
                  <div key={x} className="flex gap-3 rounded-2xl border border-zinc-100 bg-zinc-50 p-4">
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#660066] text-sm text-white">{i + 1}</span>
                    <p className="text-base leading-6 text-zinc-700">{x}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="order-1 lg:order-2">
          <div className="inline-flex items-center gap-2 text-sm uppercase tracking-[.18em] text-[#660066]"><Sparkles size={16} /> AI intelligence</div>
          <h2 className="mt-4 text-5xl tracking-[-0.045em] sm:text-6xl">{copy.aiTitle}</h2>
      ...
      <section id="sources" className="border-y border-zinc-200 bg-zinc-50 py-20">
        <div className="mx-auto max-w-7xl px-6 text-center">
          ...
          <h2 className="mt-5 text-5xl tracking-[-0.04em]">{copy.sources}</h2>
      ...
      <section id="request-demo" className="scroll-mt-28 bg-white py-24">
        <div className="mx-auto grid max-w-7xl items-start gap-12 px-6 lg:grid-cols-[.9fr_1.1fr]">
          <div className="lg:pt-10">
            <div className="inline-flex items-center gap-2 rounded-full bg-[#660066]/8 px-4 py-2 text-sm text-[#660066]">
              <MessageCircleMore size={16} />
              {copy.demo}
            </div>
            <h2 className="mt-5 text-5xl tracking-[-0.045em] sm:text-6xl">{copy.demoTitle}</h2>
            <p className="mt-5 max-w-xl text-xl leading-8 text-zinc-600">{copy.demoP}</p>
          </div>
