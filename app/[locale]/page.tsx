    ...
    <main>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-14 px-6 py-20 lg:grid-cols-2 lg:py-28">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href={`/${locale}/signup`}
                className="inline-flex items-center gap-2 rounded-full bg-metrix-900 px-6 py-3.5 font-bold text-white"
              >
                {t.hero.start}
                <Arrow size={18} />
              </a>
              <a
                href="#features"
                className="rounded-full border border-zinc-200 bg-white px-6 py-3.5 font-bold text-zinc-800"
              >
                {t.hero.explore}
              </a>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {t.features.items.map((item: string[], i: number) => {
              const Icon = icons[i];
              return (
                <div key={item[0]} className="rounded-3xl border border-black/5 bg-zinc-50 p-7">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-metrix-900 text-white">
                    <Icon size={22} />
                  </div>
                  <h3 className="mt-5 text-xl font-black">{item[0]}</h3>
                  <p className="mt-3 leading-7 text-zinc-600">{item[1]}</p>
                </div>
              );
            })}
          </div>
      ...
      <section id="how" className="py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid gap-10 lg:grid-cols-3">
            {t.how.steps.map((step: string[], i: number) => (
              <div key={step[0]} className="rounded-3xl bg-metrix-950 p-8 text-white">
                <div className="text-sm font-black tracking-[0.2em] text-metrix-300">
                  0{i + 1}
                </div>
                <h3 className="mt-6 text-2xl font-black">{step[0]}</h3>
                <p className="mt-3 leading-7 text-white/70">{step[1]}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="insights" className="bg-metrix-950 py-20 text-white">
        <div className="mx-auto max-w-7xl px-6 text-center">
          <p className="text-sm font-black uppercase tracking-[0.2em] text-metrix-300">
            {t.insights.eyebrow}
          </p>
      ...
      <section id="pricing" className="py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="rounded-[2rem] border border-metrix-100 bg-metrix-50 p-10 text-center md:p-14">
            <h2 className="text-4xl font-black">{t.pricing.title}</h2>
            <p className="mx-auto mt-4 max-w-2xl leading-8 text-zinc-600">
              {t.pricing.description}
            </p>
            <a
              href={`/${locale}/signup`}
              className="mt-7 inline-flex rounded-full bg-metrix-900 px-7 py-3.5 font-bold text-white"
            >
              {t.pricing.button}
            </a>
          </div>
