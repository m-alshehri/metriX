...
export default async function ResetPasswordPage({params,searchParams}:{params:{locale:string};searchParams?:{error?:string}}){
  ...
  return <main dir={ar?"rtl":"ltr"} className="flex min-h-screen items-center justify-center bg-zinc-50 px-6">
    <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-soft">
      <Link href={`/${locale}`} className="text-3xl font-extrabold text-metrix-900">metriX</Link>
      <h1 className="mt-8 text-4xl font-extrabold">{ar?"تعيين كلمة مرور جديدة":"Choose a new password"}</h1>
      {!user?<><div className="mt-5 rounded-2xl bg-amber-50 p-4 text-base text-amber-900">{ar?"رابط الاستعادة غير صالح أو انتهت صلاحيته. اطلب رابطًا جديدًا.":"This recovery link is invalid or expired. Request a new one."}</div><Link href={`/${locale}/forgot-password`} className="mt-5 block text-center font-semibold text-metrix-900 hover:underline">{ar?"طلب رابط جديد":"Request a new link"}</Link></>:<>
        <p className="mt-3 text-base text-zinc-600">{ar?"اكتب كلمة المرور الجديدة مرتين للتأكيد.":"Enter your new password twice to confirm it."}</p>
