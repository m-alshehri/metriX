...
export function contentHash(
  ...
  return createHash("sha256")
    .update(
      `${String(platform).toLowerCase()}|${String(author || "").toLowerCase()}|${String(
        content || "",
      )
        .trim()
        .toLowerCase()}`,
...
export function extractTextMetadata(content: string) {
  ...
  const hashtags = Array.from(
    new Set(
      (text.match(/#[A-Za-z0-9_\u0600-\u06FF]+/g) || []).map((x) =>
...
export function detectLanguageHeuristic(content: string) {
  ...
  if (
    arabic &&
    latin &&
    Math.min(arabic, latin) / Math.max(arabic, latin) > 0.18
  )
    return "mixed";
...
export function inferMedia(raw: any) {
  ...
  const mediaType =
    raw?.video || raw?.video_url || raw?.play_count !== undefined
      ? "video"
      : mediaUrl
        ? "image"
        : null;
  const mediaCount = Array.isArray(raw?.carousel_media)
    ...
    verified:
      typeof user?.verified === "boolean"
        ...
        : typeof user?.is_verified === "boolean"
          ...
          : null,
    biography:
      firstString(user?.biography, user?.bio, user?.description) || null,
    category:
      firstString(user?.category, user?.category_name, user?.account_type) ||
      null,
    location:
      firstString(
        user?.location,
...
export function recoverContent(raw: any, fallback: string) {
  ...
  function walk(v: any, depth = 0): string {
    ...
    for (const val of Object.values(v)) {
      const x = walk(val, depth + 1);
      if (x) return x;
    }
    return "";
  }
  const current = String(fallback || "").trim();
  if (current && !/^\[[^\]]+(post|video|review)[^\]]*\]$/i.test(current))
    return current;
